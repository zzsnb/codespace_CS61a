...
def accuracy(typed, source):
    ...
    if typed_length == 0 and source_length == 0:
        return 100.0
    elif typed_length == 0 and source_length != 0:
        return 0.0
    elif typed_length != 0 and source_length == 0:
        return 0.0

    for i in range(0, min(typed_length, source_length)):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    # 先搜索是否已经有该单词了，如果有，直接返回
    if len(word_list) < 1:
        return typed_word
    if typed_word in word_list:
        return typed_word
    # 如果没有，word_list当中每个词的diff并取最小的那个
    min_diff = limit + 1
    minDifWord = ''
    for word in word_list:
        miner = min(min_diff, diff_function(typed_word, word, limit), key=abs)
        if miner != min_diff :
            minDifWord = word
            min_diff = miner
    # 如果其大于limit，return typed_word
    if min_diff > limit:
        return typed_word
    else:
        return minDifWord
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    # 如果长度差距已经大于limit， 直接返回limit + 1
    if (abs(len(typed) - len(source)) > limit):
        return limit + 1
    
    # 设计一个辅助函数count，计算需要改多少个词 
    # 在递归的过程中，如果发现已经比limit还大了，直接return.
    diff = abs(len(typed) - len(source))
    def count(_typed, _source):
        nonlocal diff
        if diff > limit:
            return
        if _typed == '' or _source == '':
            return
        if _typed[0] != _source[0]:
            diff += 1
            count(_typed[1:], _source[1:])
        else:
            diff += 0
            count(_typed[1:], _source[1:])
        return
...
def minimum_mewtations(typed, source, limit):
    ...
    # 递归性地使用三种方法进行修改
    # base case
    def count(_typed, _source, current_edits):
        if _typed == '':
            return len(_source)
        elif _source == '':
            return len(_typed)
        if current_edits > limit:
            return limit + 1
        
        if _typed[0] == _source[0]:
            return count(_typed[1:], _source[1:], current_edits)
        else:
            add_cost = 1 + count(_typed, _source[1:], current_edits + 1)
            # 删除操作：删除 typed[0]
            remove_cost = 1 + count(_typed[1:], _source, current_edits + 1)
            # 替换操作：替换 typed[0] 为 source[0]
            substitute_cost = 1 + count(_typed[1:], _source[1:], current_edits + 1)
            return min(add_cost, remove_cost, substitute_cost)
    result = count(typed, source, 0)
    return result
...
def final_diff(typed, source, limit):
    ...
    # 设置一个miner_diff = 0.9，在两个字母连续时，两个位置的字母替换时，使用miner_diff
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(0, min(typed_length, source_length)):
        if typed[i] != source[i]:
            progress = i / source_length
            upload({'id' : user_id, 'progress' : progress})
            return progress
    progress = typed_length / source_length
    upload({'id' : user_id, 'progress' : progress})
    return progress
...
def time_per_word(words, timestamps_per_player):
    ...
    times = [
        ...
        for player in tpp  # 直接遍历每个玩家的时间戳列表
    ]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for word_index in word_indices:
        ...
        for player_index in player_indices:
            tempTime = get_time(times, player_index, word_index)
            if get_time(times, player_index, word_index) < min_time:
                min_player = player_index
                min_time = tempTime
        result[min_player].append(words[word_index])
    return result
    # END PROBLEM 10
